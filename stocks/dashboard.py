...
def generate_master_dashboard_html(watchlist: Dict[str, WatchlistStock], alerts: List[AlertItem]) -> str:
    ...
    if not watchlist:
        table_rows_html = """
        <tr>
            <td colspan="5" style="text-align: center; padding: 60px 24px; color: var(--text-secondary); font-family: var(--font-serif);">
                <div style="color: var(--text-title); font-size: 1.15rem; font-weight: 500; margin-bottom: 4px;">No Stocks Tracked</div>
                <div style="font-size: 0.9rem; color: var(--text-secondary);">Ready for coverage.</div>
            </td>
        </tr>
        """
        grid_cards_html = """
        <div style="grid-column: 1 / -1; text-align: center; padding: 60px 24px; background: var(--bg-panel); border: 1px dashed var(--border-color); border-radius: 14px;">
            <div style="color: var(--text-title); font-size: 1.15rem; font-weight: 500; margin-bottom: 4px;">No Stocks Tracked</div>
            <div style="color: var(--text-secondary); font-size: 0.9rem;">Ready for coverage.</div>
        </div>
        """
